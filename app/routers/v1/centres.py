...
async def create_centre(body: CentreCreate, db: AsyncSession = Depends(get_db)) -> CentreResponse:
...
async def get_centre(centre_id: uuid.UUID, db: AsyncSession = Depends(get_db)) -> CentreResponse:
...
async def delete_centre(
    centre_id: uuid.UUID, db: AsyncSession = Depends(get_db)
) -> SuccessResponse:
    centre = await centre_service.get_by_id(db, centre_id)
